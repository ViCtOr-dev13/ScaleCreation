...
export function parseLinearGradientString(
  ...
  const radians = ((angle - 90) * Math.PI) / 180; // -90 pour corriger l'angle
  ...
  return new fabric.Gradient({
    type: "linear",
    coords: {
      x1: canvas.width! / 2 - Math.cos(radians) * canvas.width! / 2,
      y1: canvas.height! / 2 - Math.sin(radians) * canvas.height! / 2,
      x2,
      y2,
    },
    colorStops,
