...
const LinkedinEditor = ({ userId = "default-user-id" }: LinkedinEditorProps)  => {
  ...
  const onLaunchCreation = async () => {
    ...
    try {
      ...
      alert("Erreur lors de la création du post Linkedin");
    }
  };
  ...
  return (
      <div className="w-full xl:w-5xl root:bg-white  rounded-3xl shadow-xl border-2 border-black dark:border-white p-12">
        <h1 className="text-3xl mb-12 text-center">
          Génerer un post Linkedin optimisé
        </h1>
        ...
        <div className="mb-8">
          ...
          <textarea
            placeholder={`Expliquer l'objectif de votre post Linkedin, de quoi ça parle
la trame que vous voulez utiliser :  story telling, annonce choc, explication clair et detaillé...`}
            value={prompt}
